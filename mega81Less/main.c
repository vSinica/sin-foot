...
{
	PORTD &= ~0b00000010; // Отпускаем ножку направления PD2 по прерыванию завершения отправки данных
	UCSR0B &= ~(1 << TXCIE0);
	UCSR0B |= (1 << RXCIE0);
}
...
{
	PORTD |= 0b00000010; // Поднимаем ножку направления PD2 по прерыванию начала отправки данных
	if(tx_index < 5){ 	// Вывели весь буффер?
